...
export enum OrderPlaceV1_OrderPlacementStatus {
   ...
   * placed on the Vindax node sending the off-chain update message.
   * The cases where this happens includes:
   * - The Vindax node places an order in it's in-memory orderbook during the
   *   CheckTx flow.
   * A best effort placed order may not have been placed on other Vindax
   * nodes including other Vindax validator nodes and may still be excluded in
   * future order matches.
   */
  ORDER_PLACEMENT_STATUS_BEST_EFFORT_OPENED = 1,

  /**
   * ORDER_PLACEMENT_STATUS_OPENED - An opened order is one that is confirmed to be placed on all Vindax nodes
   * (discounting dishonest Vindax nodes) and will be included in any future
   * order matches.
...
export enum OrderRemoveV1_OrderRemovalStatus {
   ...
   * removed on the Vindax node sending the off-chain update message.
   * The cases where this happens includes:
   * - the order was removed due to the Vindax node receiving a CancelOrder
   *   transaction for the order.
   * - the order was removed due to being undercollateralized during
   *   optimistic matching.
   * A best effort canceled order may not have been removed on other Vindax
   * nodes including other Vindax validator nodes and may still be included in
   * future order matches.
   */
  ORDER_REMOVAL_STATUS_BEST_EFFORT_CANCELED = 1,

  /**
   * ORDER_REMOVAL_STATUS_CANCELED - A canceled order is one that is confirmed to be removed on all Vindax nodes
   * (discounting dishonest Vindax nodes) and will not be included in any future
   * order matches.
   * The cases where this happens includes:
