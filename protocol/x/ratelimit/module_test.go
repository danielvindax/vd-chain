...
import (
	...
	"github.com/danielvindax/vd-chain/protocol/x/ratelimit"
	ratelimit_keeper "github.com/danielvindax/vd-chain/protocol/x/ratelimit/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	registeredRoutes := []string{
		"/vindax/v4/ratelimit/list_limit_params",
		"/vindax/v4/ratelimit/capacity_by_denom",
	}
	...
	recorder := httptest.NewRecorder()
	req, err := http.NewRequest("GET", "/vindax/v4/ratelimit/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
