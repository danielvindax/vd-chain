...
SNAP_PATH="/vindax/chain/.full-node-2/snapshots/vindax/"
# logfile containing snapshot timestamps
LOG_PATH="/vindax/chain/.full-node-2/snapshots/vindax/vindax_log.txt"
# data directory to snapshot. this contains the blockchain state.
DATA_PATH="/vindax/chain/.full-node-2/data/"
...
sed -i 's/pruning = "default"/pruning = "everything"/' /vindax/chain/.full-node-2/config/app.toml
...
sed -i 's/min-retain-blocks = 0/min-retain-blocks = 2/' /vindax/chain/.full-node-2/config/app.toml
# Do not index tx_index.db
sed -i 's/indexer = "kv"/indexer = "null"/' /vindax/chain/.full-node-2/config/config.toml
...
while true; do
  ...
  cosmovisor run start --log_level info --home /vindax/chain/.full-node-2 --p2p.seeds "${p2p_seeds}" --non-validating-full-node=true --dd-agent-host=${dd_agent_host} &
