// This file is @generated by prost-build.
pub mod cosmos {
    pub mod app {
        pub mod runtime {
            pub mod v1alpha1 {
                include!("cosmos.app.runtime.v1alpha1.rs");
            }
        }
        pub mod v1alpha1 {
            include!("cosmos.app.v1alpha1.rs");
        }
    }
    pub mod auth {
        pub mod module {
            pub mod v1 {
                include!("cosmos.auth.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.auth.v1beta1.rs");
        }
    }
    pub mod authz {
        pub mod module {
            pub mod v1 {
                include!("cosmos.authz.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.authz.v1beta1.rs");
        }
    }
    pub mod autocli {
        pub mod v1 {
            include!("cosmos.autocli.v1.rs");
        }
    }
    pub mod bank {
        pub mod module {
            pub mod v1 {
                include!("cosmos.bank.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.bank.v1beta1.rs");
        }
    }
    pub mod base {
        pub mod abci {
            pub mod v1beta1 {
                include!("cosmos.base.abci.v1beta1.rs");
            }
        }
        pub mod node {
            pub mod v1beta1 {
                include!("cosmos.base.node.v1beta1.rs");
            }
        }
        pub mod query {
            pub mod v1beta1 {
                include!("cosmos.base.query.v1beta1.rs");
            }
        }
        pub mod reflection {
            pub mod v1beta1 {
                include!("cosmos.base.reflection.v1beta1.rs");
            }
            pub mod v2alpha1 {
                include!("cosmos.base.reflection.v2alpha1.rs");
            }
        }
        pub mod tendermint {
            pub mod v1beta1 {
                include!("cosmos.base.tendermint.v1beta1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.base.v1beta1.rs");
        }
    }
    pub mod circuit {
        pub mod module {
            pub mod v1 {
                include!("cosmos.circuit.module.v1.rs");
            }
        }
        pub mod v1 {
            include!("cosmos.circuit.v1.rs");
        }
    }
    pub mod consensus {
        pub mod module {
            pub mod v1 {
                include!("cosmos.consensus.module.v1.rs");
            }
        }
        pub mod v1 {
            include!("cosmos.consensus.v1.rs");
        }
    }
    pub mod crisis {
        pub mod module {
            pub mod v1 {
                include!("cosmos.crisis.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.crisis.v1beta1.rs");
        }
    }
    pub mod crypto {
        pub mod ed25519 {
            include!("cosmos.crypto.ed25519.rs");
        }
        pub mod hd {
            pub mod v1 {
                include!("cosmos.crypto.hd.v1.rs");
            }
        }
        pub mod keyring {
            pub mod v1 {
                include!("cosmos.crypto.keyring.v1.rs");
            }
        }
        pub mod multisig {
            include!("cosmos.crypto.multisig.rs");
            pub mod v1beta1 {
                include!("cosmos.crypto.multisig.v1beta1.rs");
            }
        }
        pub mod secp256k1 {
            include!("cosmos.crypto.secp256k1.rs");
        }
        pub mod secp256r1 {
            include!("cosmos.crypto.secp256r1.rs");
        }
    }
    pub mod distribution {
        pub mod module {
            pub mod v1 {
                include!("cosmos.distribution.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.distribution.v1beta1.rs");
        }
    }
    pub mod evidence {
        pub mod module {
            pub mod v1 {
                include!("cosmos.evidence.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.evidence.v1beta1.rs");
        }
    }
    pub mod feegrant {
        pub mod module {
            pub mod v1 {
                include!("cosmos.feegrant.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.feegrant.v1beta1.rs");
        }
    }
    pub mod genutil {
        pub mod module {
            pub mod v1 {
                include!("cosmos.genutil.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.genutil.v1beta1.rs");
        }
    }
    pub mod gov {
        pub mod module {
            pub mod v1 {
                include!("cosmos.gov.module.v1.rs");
            }
        }
        pub mod v1 {
            include!("cosmos.gov.v1.rs");
        }
        pub mod v1beta1 {
            include!("cosmos.gov.v1beta1.rs");
        }
    }
    pub mod group {
        pub mod module {
            pub mod v1 {
                include!("cosmos.group.module.v1.rs");
            }
        }
        pub mod v1 {
            include!("cosmos.group.v1.rs");
        }
    }
    pub mod mint {
        pub mod module {
            pub mod v1 {
                include!("cosmos.mint.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.mint.v1beta1.rs");
        }
    }
    pub mod nft {
        pub mod module {
            pub mod v1 {
                include!("cosmos.nft.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.nft.v1beta1.rs");
        }
    }
    pub mod orm {
        pub mod module {
            pub mod v1alpha1 {
                include!("cosmos.orm.module.v1alpha1.rs");
            }
        }
        pub mod query {
            pub mod v1alpha1 {
                include!("cosmos.orm.query.v1alpha1.rs");
            }
        }
        pub mod v1 {
            include!("cosmos.orm.v1.rs");
        }
        pub mod v1alpha1 {
            include!("cosmos.orm.v1alpha1.rs");
        }
    }
    pub mod params {
        pub mod module {
            pub mod v1 {
                include!("cosmos.params.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.params.v1beta1.rs");
        }
    }
    pub mod reflection {
        pub mod v1 {
            include!("cosmos.reflection.v1.rs");
        }
    }
    pub mod slashing {
        pub mod module {
            pub mod v1 {
                include!("cosmos.slashing.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.slashing.v1beta1.rs");
        }
    }
    pub mod staking {
        pub mod module {
            pub mod v1 {
                include!("cosmos.staking.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.staking.v1beta1.rs");
        }
    }
    pub mod store {
        pub mod internal {
            pub mod kv {
                pub mod v1beta1 {
                    include!("cosmos.store.internal.kv.v1beta1.rs");
                }
            }
        }
        pub mod snapshots {
            pub mod v1 {
                include!("cosmos.store.snapshots.v1.rs");
            }
        }
        pub mod streaming {
            pub mod abci {
                include!("cosmos.store.streaming.abci.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.store.v1beta1.rs");
        }
    }
    pub mod tx {
        pub mod config {
            pub mod v1 {
                include!("cosmos.tx.config.v1.rs");
            }
        }
        pub mod signing {
            pub mod v1beta1 {
                include!("cosmos.tx.signing.v1beta1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.tx.v1beta1.rs");
        }
    }
    pub mod upgrade {
        pub mod module {
            pub mod v1 {
                include!("cosmos.upgrade.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.upgrade.v1beta1.rs");
        }
    }
    pub mod vesting {
        pub mod module {
            pub mod v1 {
                include!("cosmos.vesting.module.v1.rs");
            }
        }
        pub mod v1beta1 {
            include!("cosmos.vesting.v1beta1.rs");
        }
    }
}
pub mod cosmos_proto {
    include!("cosmos_proto.rs");
}
pub mod google {
    pub mod api {
        include!("google.api.rs");
        pub mod expr {
            pub mod v1alpha1 {
                include!("google.api.expr.v1alpha1.rs");
            }
            pub mod v1beta1 {
                include!("google.api.expr.v1beta1.rs");
            }
        }
    }
    pub mod bytestream {
        include!("google.bytestream.rs");
    }
    pub mod geo {
        pub mod r#type {
            include!("google.geo.r#type.rs");
        }
    }
    pub mod longrunning {
        include!("google.longrunning.rs");
    }
    pub mod r#type {
        include!("google.r#type.rs");
    }
    pub mod rpc {
        include!("google.rpc.rs");
        pub mod context {
            include!("google.rpc.context.rs");
        }
    }
}
pub mod tendermint {
    pub mod abci {
        include!("tendermint.abci.rs");
    }
    pub mod crypto {
        include!("tendermint.crypto.rs");
    }
    pub mod libs {
        pub mod bits {
            include!("tendermint.libs.bits.rs");
        }
    }
    pub mod p2p {
        include!("tendermint.p2p.rs");
    }
    pub mod types {
        include!("tendermint.types.rs");
    }
    pub mod version {
        include!("tendermint.version.rs");
    }
}
pub mod vindax {
    pub mod accountplus {
        include!("vindax.accountplus.rs");
    }
    pub mod affiliates {
        include!("vindax.affiliates.rs");
    }
    pub mod assets {
        include!("vindax.assets.rs");
    }
    pub mod blocktime {
        include!("vindax.blocktime.rs");
    }
    pub mod bridge {
        include!("vindax.bridge.rs");
    }
    pub mod clob {
        include!("vindax.clob.rs");
    }
    pub mod daemons {
        pub mod bridge {
            include!("vindax.daemons.bridge.rs");
        }
        pub mod liquidation {
            include!("vindax.daemons.liquidation.rs");
        }
        pub mod pricefeed {
            include!("vindax.daemons.pricefeed.rs");
        }
    }
    pub mod delaymsg {
        include!("vindax.delaymsg.rs");
    }
    pub mod epochs {
        include!("vindax.epochs.rs");
    }
    pub mod feetiers {
        include!("vindax.feetiers.rs");
    }
    pub mod govplus {
        include!("vindax.govplus.rs");
    }
    pub mod indexer {
        pub mod events {
            include!("vindax.indexer.events.rs");
        }
        pub mod indexer_manager {
            include!("vindax.indexer.indexer_manager.rs");
        }
        pub mod off_chain_updates {
            include!("vindax.indexer.off_chain_updates.rs");
        }
        pub mod protocol {
            pub mod v1 {
                include!("vindax.indexer.protocol.v1.rs");
            }
        }
        pub mod redis {
            include!("vindax.indexer.redis.rs");
        }
        pub mod shared {
            include!("vindax.indexer.shared.rs");
        }
        pub mod socks {
            include!("vindax.indexer.socks.rs");
        }
    }
    pub mod listing {
        include!("vindax.listing.rs");
    }
    pub mod perpetuals {
        include!("vindax.perpetuals.rs");
    }
    pub mod prices {
        include!("vindax.prices.rs");
    }
    pub mod ratelimit {
        include!("vindax.ratelimit.rs");
    }
    pub mod revshare {
        include!("vindax.revshare.rs");
    }
    pub mod rewards {
        include!("vindax.rewards.rs");
    }
    pub mod sending {
        include!("vindax.sending.rs");
    }
    pub mod stats {
        include!("vindax.stats.rs");
    }
    pub mod subaccounts {
        include!("vindax.subaccounts.rs");
    }
    pub mod vault {
        include!("vindax.vault.rs");
    }
    pub mod vest {
        include!("vindax.vest.rs");
    }
}
